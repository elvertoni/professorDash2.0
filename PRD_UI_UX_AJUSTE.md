...
- [x] D.6. Decisao concluida: 2a passada de revisao/otimizacao UI/UX orquestrada com a skill `impeccable` (5 agentes em paralelo, 1 surface cada; mudancas em arquivos compartilhados `app.css`/`design-system.html` aplicadas centralmente para evitar race). Achados aplicados:
  - **Bug global** — KPIs usavam `.ico green/violet/yellow`, classes que nunca existiram em `app.css` nem no DS (o demo do DS usava tint inline). Icones renderizavam como caixas vazias em `home`, `aluno_dashboard` e `professor_dashboard`. Corrigido: utilitarios `.kpi .ico.green/.violet/.yellow/.cyan` adicionados ao `app.css` E ao `design-system.html` (fonte canonica), inline removido dos templates e do demo do DS.
  - **Bans do impeccable** — removidos eyebrow-por-secao + gradient-text repetidos no `aluno_dashboard` (5 secoes) e accents gradient nas partials do dashboard do professor; mantida apenas a assinatura unica de hero. `.notification-item.is-unread` trocou side-stripe banido (`inset 3px 0 0`) por top-stripe (`inset 0 2px 0`).
  - **A11y** — filtro `aria_field` (`base/templatetags/form_extras.py`) injeta `aria-invalid`/`aria-describedby` no partial compartilhado `form_fields.html` (usado por 14 templates); `aria-hidden` em ~40 icones decorativos faltantes; `aria-label` no select de publicacao; active-state dinamico no `profile_base`.
  - **Nav** — conditionals de active-state mortos removidos do `base.html` (`'#jornada' in path`, `'/relatorio/' in path` — rota inexistente). "Relatorios" agora ancora em `turma_detail#relatorios`.
  - **Refactor anti-slop** — `home` perdeu a secao de dev-scaffolding (`/health/`, "Sprint 0", PostgreSQL) + barra inline hardcoded; copy de aluno de-jargonizada. `turma_detail` teve ~12 inline-styles estaticos trocados por classes documentadas `.action-grid/.action-panel/.danger-zone` (+ `.empty .btn`, `.card .card-head .tag-disc{min-width:0}`).
...
- [x] D.8. Decisao concluida: 3a passada Impeccable orquestrada por jornada (aluno, professor e sistema/design). ~~O painel do aluno conserva aulas de hoje também na aba Aulas~~ (**revertido em D.11** — a duplicata gerava dois badges e dois verbos para a mesma aula) e abre a primeira aba acionável; tabs ganharam `x-cloak`, navegação circular e estado ARIA coerente. O leitor prioriza a próxima aula após a conclusão e deixa desfazer como ação secundária. A grade de checks recebeu cabeçalho e barra de salvamento sticky, modal nativo e indicador de alterações. Contraste de ciano/info no tema claro, skip link, labels de ações por ícone, escopo de cabeçalhos Markdown e loaders com reduced motion foram reforçados. Componentes recorrentes (`card-cover-fallback`, modal e botões semânticos) e os bans de side-stripe/gradient text foram sincronizados com o Design System canônico.
...
- [x] D.11. Decisao concluida: critique Impeccable do painel do aluno (`templates/classroom/aluno_dashboard.html`), pontuado 19/40 nas heuristicas de Nielsen, seguido de correcao orquestrada em quatro frentes com propriedade exclusiva de arquivo e revisao cruzada por Codex. Cascata: `.card .card-foot a` (0,2,1) vencia `[data-theme='light'] .btn-primary` (0,2,0) e pintava o rotulo do CTA do aluno com `--accent-text` sobre o proprio gradiente esmeralda (~1.1:1 no light, ~1.8:1 no dark); corrigido com `a:not(:where(.btn))` — `:where()` para nao elevar especificidade — e espelhado no design system canonico, que reintroduziria o conflito. Verdade dos numeros: o contador da aba deixou de ser `proximas|length` (capado em 6) e passou a ser `total_pendentes` real; a aula de hoje nao aparece mais em duas abas; `publicada.concluida` reflete o `ProgressoAula` em vez de `False` fixo, revivendo o ramo "Concluida"; `aula__trilha` entrou no `select_related` (era N+1 por card). Progresso agregado global removido do topo por ser hero-metric banido e inacionavel — vive agora em `turma.progresso_pct` no card da turma. Layout: hero compacto e capa fora do card de hoje trazem o primeiro CTA de y~1146 para y~498 num viewport de 360x640, dentro da dobra. Estados: `.empty` ganhou moldura (era 1.00:1 contra o fundo da pagina, identico ao `body` nos dois temas) e o trilho do `.progress` saiu de 1.07:1 — quebrado tambem no dark, nao so no light — com a fronteira fill/track em >=3:1; a regra `[data-theme='light'] .progress i` era no-op e foi removida. A celebracao "Tudo em dia por hoje!" deixou de disparar com pendencias acumuladas. Resiliencia: Alpine e Lucide saíram do `unpkg` (o `lucide@latest` ja redirecionava para a major 1.34.0 em producao, sem deploy) para `static/js/vendor/` — Alpine 3.14.1 integral e subset gerado de 137 icones do Lucide, 29KB contra 419KB — com fallback reversivel `html.alpine-failed` e `<noscript>` que revelam os paineis empilhados em vez de pagina em branco. Sombra do `.btn-primary` reconciliada entre design system e runtime: repouso tintado, hover elevando e `scale` movido para `:active`, com `box-shadow` declarado uma unica vez em (0,1,0) para que componentes mais especificos vencam por cascata normal, e zerada dentro de `.card`/`.tbl-wrap`, que recortariam a sombra. Faixa de abas mantida deliberadamente contra recomendacao do Codex: lista unica renderia ~5.500px de rolagem a 360px, o oposto de "uma decisao por tela". Validacao manual na VPS pendente para o humano.

- [x] D.12. Decisao concluida: 2a passada de critique no painel do aluno (19/40 -> 23/40) revelou que duas das nove correcoes de D.11 nao entraram como reportado, e que a propria rodada gerou duas regressoes. O contador da aba "Aulas" continuava imprimindo `proximas|length` (capado em 6) enquanto o painel ao lado exibia o backlog real — dois numeros contraditorios na mesma tela; e o corte `pendentes[:6]` sobre lista ordenada por `-disponivel_em` entregava as pendentes MAIS RECENTES, escondendo as antigas, que sao justamente as que a turma ja cobriu. Alem disso, como `proximas` passava a conter concluidas quando o backlog zerava, o aluno que zerava tudo aterrissava numa lista do que ja tinha feito, com a celebracao conquistada escondida atras de uma aba sem badge. **A aba intermediaria foi removida**: o painel passa a ter duas abas (Hoje / Minhas turmas), o contador exibido e sempre o numero real (`total_pendentes`), o backlog vive no card da turma (`turma.total_pendentes`) que tem a lista inteira e ordenada em `AlunoTurmaAulasView`, e o default de aba deixa de poder cair numa lista de aulas ja feitas. O argumento que sustentou as tres abas em D.11 — "lista unica renderia ~5.500px" — foi verificado e estava inflado: o pior caso real e ~3.400px. Corrigida tambem a concordancia de "as N aulas disponiveis", que rendia "as 1 aula disponiveis" no singular. Regressao de icone introduzida por paralelizacao em D.11 (`calendar-clock` e `list-todo` ausentes do subset vendorizado do Lucide, usados so nos estados vazios novos) corrigida com regeracao para 139 icones; o deck de apresentacao, que nao estende `base.html`, tambem saiu do unpkg; e `github`, removido do Lucide 1.34.0, foi trocado por `download` no botao de import do acervo. Pendencias registradas e nao atacadas: heuristica 10 (ajuda/documentacao) segue em 1/4 nas duas passadas; Google Fonts continua render-blocking em `base.html:12` pelo mesmo tipo de CDN que motivou vendorizar o Alpine; os botoes de troca de aba ficam inertes no fallback `html.alpine-failed`; e `.badge-warning` reprova AA no tema claro (~4,47:1) por usar amber de tema escuro hardcoded sem par claro. Validacao manual na VPS pendente para o humano.
...
- [x] D.15. Auditoria UX/UI multi-agente + correcao em 5 lotes (L0-L4). **L0 sistemico**: `padding-top` de pagina reduzido de `clamp(36,6vw,84)` p/ `clamp(20,3vw,40)` (faixa morta ~130px -> ~50px); `body` vira flex-column + `#conteudo` flex (footer ancorado, sem scroll morto); tema light re-tunado (`--dim` #8e8e95 -> #6b6b72; `--accent-text` -> `--signal-text`; titulos de callout usam variantes `-deep` no light) p/ passar WCAG AA; `.classroom-page .section-heading h1` volta ao Archivo (estava em mono por seletor esquecido); escala de titulo interna cai de 84px p/ 52px; ornamento fora do canon Volta removido (washes radiais do `body`, `body::before` regua/spotlight, glass do header, stripe vermelha animada, blur do navchain); piso de fonte 11-12px em chips/badges/navchain. **L1 aula**: `.lesson-actionbar` fica opaca (era glass 86% e o texto vazava por tras) e no mobile mostra so a acao primaria + link "desfazer" discreto; `--reader-grad` (indefinido -> barra de progresso da Leitura Projetada invisivel na TV) definido; `strip_redundant_lead()` em `catalog/parser.py` remove o H1/H2 lider + paragrafo == `tema` do corpo armazenado (eliminava dois `<h1>` na pagina — assume `canonica.md` comeca com `# {titulo}`); corpo de aula sobe p/ piso 18px, `line-height` 1.8, `word-spacing` .02em, medida 58ch (mitigacao de leitura longa 14-18 TDAH mantendo JetBrains Mono, que e canon do DS); capa do reader fica neutra (era esmeralda+vermelho+ambar sem funcao). **L2 professor**: grade de checks e tabelas densas ganham sombra de rolagem horizontal (token, visivel nos dois temas) + hint "deslize" + `role=region`/`tabindex` no mobile; `.check-grid__actionbar` deixa de ser sticky sobre as linhas; "(opcional) (opcional)" duplicado removido (labels hardcoded em `activities`/`materials` forms + partial nao poe sufixo em checkbox); separador de breadcrumb entre TODOS os itens; `input[type=file]` estilizado com o DS; `<h3>` da zona de perigo vira `<h2>`, tabelas ganham `<caption>`. **L3 aluno**: aba "Hoje" lidera sempre que ha pendencia (nao so quando algo foi liberado na data); badge de pendencias move p/ "Hoje" como pilula ambar distinta, "Minhas turmas" mostra a contagem real de turmas; turmas homonimas distinguidas pelo codigo de convite no card; ordenacao pendente-primeiro; aluno nao edita mais o proprio `email` (readonly + aviso "definido pela escola" — alterar desassociava a matricula); `widthratio` protegido contra divisao por zero. **L4 shell/auth**: inputs de login ganham borda visivel (`--line-ui`) e fundo distinto; `.btn-primary` hover usa `--signal-deep` (#fff sobre #fb3732 reprovava); `aria-current="page"` espelhado da classe `.active` no header; menu "MENU" mobile suprimido para aluno (1 destino); ornamento de auth removido (quadrados diagonais rotacionados, stripe do painel); label "TURMA" do hero da home nao corta mais; botao "Sair" da sidebar de perfil deixa de parecer desabilitado (fundo transparente + texto `--signal-text` + separador). Validacao: `manage.py check` OK, detector impeccable limpo, screenshots desktop/mobile + claro/escuro por lote. **Follow-up pendente** (redesign, nao mecanico): reordenar aulas com drag sem reload (B2); IA da tela `turma_detail` — 4 paineis de peso igual, 10 botoes antes da lista no mobile (B6); controle segmentado "Publicar agora / Agendar / Ocultar" (B7); `confirm()` nativo -> `<dialog>` do DS em Sincronizar/renovar (B8); consolidacao das 3 geracoes de regras em `app.css` (S7); swap parcial p/ toggle de conclusao e save de checks (S8); rail lateral da aula com funcao real ou removido (S9).
