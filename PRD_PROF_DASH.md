...
- [x] **Modo apresentação** (professor/admin): aula em tela cheia para projetar na sala (TV 42" / Educatron). Rota `classroom:aula_apresentar` (`/turmas/<turma_pk>/aulas/<pk>/apresentar/`), template standalone `aula_presentation.html` (fora do `base.html`, sem header/nav). Navegação por slides semânticos e glanceáveis (setas/PageUp-Down/Espaço/Home/End, clique no palco, controles anterior/próximo, zonas laterais e controle remoto), tela cheia (`F`), roteiro docente em painel (`N`, reusa `teacher_notes_html`), `Esc` sai. **Decisões**: (a) slicing feito **client-side** (`static/js/deck.js`) fatiando a `.prose` por `<h2>` e fronteiras de bloco; conteúdo indivisível que exceda o piso de escala ganha rolagem interna, sem truncamento; (b) **sem reveal.js** — JavaScript/CSS nativos do design system (evita dependência/build e componentes fora do DS); (c) tema alto-contraste fixo e escala tipográfica única por viewport para leitura consistente à distância; (d) escopo professor-only na v1; (e) capa ampliada usa `<dialog>` nativo com gestão de foco e `Esc`.
