...
- [x] **Modo apresentação** (professor/admin): aula em tela cheia para projetar na sala (TV 42" / Educatron). Rota `classroom:aula_apresentar` (`/turmas/<turma_pk>/aulas/<pk>/apresentar/`), template standalone `aula_presentation.html` (fora do `base.html`, sem header/nav). A aula permanece um artigo cinematográfico contínuo conduzido por `static/js/reader.js`: setas/PageUp-Down/Espaço avançam ou voltam cerca de uma tela, Home/End alcançam as extremidades, `F` alterna tela cheia, `B` abre a Pausa Pedagógica, `N` abre o roteiro docente e `?` recupera a ajuda. A coluna textual usa `64ch`, títulos `<h2>` funcionam como landmarks e blocos didáticos/mídia preservam largura ampliada. Quizzes interativos podem revelar a próxima resposta pendente por `R` ou pelo controle visível, sem registrar a revelação como resposta ou alterar o placar; o clique normal continua disponível. **Decisões**: (a) conteúdo nunca é fatiado, truncado ou escalado por bloco — a rolagem contínua preserva integralmente a `canonica.md`; (b) **sem reveal.js** — JavaScript/CSS nativos do design system (evita dependência/build e componentes fora do DS); (c) tema alto-contraste fixo e escala tipográfica única por viewport para leitura consistente a 4–6 m; (d) escopo professor-only na v1; (e) capa ampliada usa `<dialog>` nativo com gestão de foco; (f) `Esc` fecha, em ordem, lightbox, roteiro, pausa e tela cheia antes de sair da rota.
