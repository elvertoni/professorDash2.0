...
- [x] **Modo apresentação** (professor/admin): aula em tela cheia para projetar na sala (TV 42" / Educatron). Rota `classroom:aula_apresentar` (`/turmas/<turma_pk>/aulas/<pk>/apresentar/`), template standalone `aula_presentation.html` (fora do `base.html`, sem header/nav). Navegação por slides (setas/PageUp-Down/Espaço/Home/End, zonas de clique, controle remoto), tela cheia (`F`), roteiro docente em overlay (`N`, reusa `teacher_notes_html`), `Esc` sai. **Decisões**: (a) slicing em slides feito **client-side** (`static/js/deck.js`) fatiando o `.prose` por `<h2>` — zero mudança no parser/backend; (b) **sem reveal.js** — Alpine/CSS nativos do design system (evita dependência/build e componentes fora do DS); (c) tema alto-contraste fixo e tipografia escalada por viewport (`clamp`/`vmin`) para leitura à distância; (d) escopo professor-only na v1.
