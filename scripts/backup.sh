#!/bin/sh
# Backup do Postgres + media protegida do ProfessorDash.
#
...
MEDIA_DIR="${PROTECTED_MEDIA_DIR:-/app/protected_media}"
...
if [ -d "$MEDIA_DIR" ]; then
    echo "[backup] Tar da media protegida -> media-$STAMP.tar.gz"
